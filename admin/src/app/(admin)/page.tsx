...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      .from("orders")
      .select(
        "id, status, total, billing_first_name, billing_last_name, billing_email, created_at",
      )
  ...
  const totalRevenue = (
    (revenueResult.data ?? []) as { total: number }[]
  ...
  const quickLinks = [
    ...
    },
    {
      label: "Promotions",
      href: "/promotions",
      desc: "Create promotional banners",
    },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {stats.map((s) => (
          <StatCard
            ...
            label={s.label}
            value={s.value}
            subLabel={(s as { subLabel?: string }).subLabel}
            icon={s.icon}
          />
        ))}
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-3">
                      <span
                        className={`text-xs font-medium px-2 py-1 rounded-full ${STATUS_COLORS[order.status] ?? "bg-gray-100 text-gray-600"}`}
                      >
                        {order.status}
                      </span>
                    </td>
