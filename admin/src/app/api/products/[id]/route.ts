...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await adminClient.from("products").update({ ...parsed.data, updated_at: new Date().toISOString() } as never).eq("id", Number(id)).select().maybeSingle();
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const adminClient = createAdminClient();
    const { error } = await adminClient.from("products").delete().eq("id", Number(id));
    if (error) throw error;
