  ...
  const handleItemToWishList = () => {
    if (isInWishlist) {
      ...
      if (wishlistItem?.dbItemId) {
        dispatch(
          removeItemFromWishlistDb({ id: product.id, dbItemId: wishlistItem.dbItemId })
        );
      }
    } else {
      dispatch(
        addItemToWishlistDb({ ...product, status: "available", quantity: 1 })
      );
    }
  };
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <svg
                        key={star}
                        className={`fill-[#FFA645] ${star > 3 ? "fill-gray-4" : ""}`}
                        width="18"
                        height="18"
                        viewBox="0 0 18 18"
                        fill="none"
